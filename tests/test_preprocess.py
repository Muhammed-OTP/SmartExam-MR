...
from src.smartexam_mr.data.feature_engineering import engineer_features, compute_word_overlap_ratio

def test_missing_values_handled():
    df = pd.DataFrame({
        'student_answer': ["Answer 1", np.nan],
        'reference_answer': [np.nan, "Ref 2"]
    })
